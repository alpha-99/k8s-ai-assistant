...
func (r *ResourceService) getResourceInterface(resourceOrKindArg string, ns string, client dynamic.Interface, restMapper *meta.RESTMapper) (dynamic.ResourceInterface, error) {
	...
	fmt.Println(restMapping.Resource.Group + ":" + restMapping.Resource.Resource + ":" + restMapping.Resource.Version)
	// 判断资源是命名空间级别的还是集群级别的
	if restMapping.Scope.Name() == "namespace" {
		...
		if ns != "" {
			...
			ri = client.Resource(restMapping.Resource).Namespace("default")
		}
...
func (r *ResourceService) mappingFor(resourceOrKindArgs string, restMapper *meta.RESTMapper) (*meta.RESTMapping, error) {
	fullySpecifiedGVR, groupResource := schema.ParseResourceArg(resourceOrKindArgs)
	gvk := schema.GroupVersionKind{}
	fmt.Println("resourceOrKindArgs: " + resourceOrKindArgs)
	if fullySpecifiedGVR != nil {
		gvk, _ = (*restMapper).KindFor(*fullySpecifiedGVR)
	}

	if gvk.Empty() {
		fmt.Println("group resource: ", groupResource)
	...
	fullySpecifiedGVK, groupKind := schema.ParseKindArg(resourceOrKindArgs)
	...
	if err != nil {
		if meta.IsNoMatchError(err) {
			return nil, fmt.Errorf("the server dose not have a resource type %q", groupResource.Resource)
		}
		return nil, err
	}
